...
app.post('/api/chat', async (req, res) => {
  const {
    ...
    contextRollingThreshold = 32768, // User-adjusted context rolling boundary
  } = req.body;
  ...
  try {
    ...
    // 3. RAG Retrieval: Scan current prompt + up to 3 recent messages
    ...
    // 4. Combined System Instruction with strict World Rules
    const fullSystemInstruction = `${masterInstruction}\n${formattedContext}`;

    // 5. Apply Rolling Context Window calculation
    const modelLimit = getModelContextLimit(model);
    // Ensure rolling threshold does not exceed model limit
    ...
    // 8. Execute through Gemini Key Pool with automatic 429 fallback
